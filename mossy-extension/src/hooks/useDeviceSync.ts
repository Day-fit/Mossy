...
export function useDeviceSync() {
  ...
  const initializeKeySync = useCallback(
    async (vaultId: string) => {
      ...
      try {
        ...
        const message =
          e instanceof Error ? e.message : "Key sync initialization failed";
        setError(message);
        throw e;
      }
    },
    [deviceId, executeInitKeySyncRequest],
  );
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          void saveRawKey(data.vaultId, pin, new Uint8Array(rawKey)).then(
            () => resolve(),
            reject,
          );
        } catch (e) {
