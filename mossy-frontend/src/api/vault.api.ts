...
export async function executeCreateVaultRequest(vaultName: string): Promise<VaultRegistrationDto> {
...
export async function executeDeleteVaultRequest(vaultId: string): Promise<ServerResponseDto> {
...
export async function executeUpdateVaultRequest(vaultId: string, vaultName: string): Promise<ServerResponseDto> {
    const response = await apiFetch(`/api/v1/passwords/vault/${vaultId}`, {
        method: "PUT",
        body: JSON.stringify({ vaultName }),
    });
