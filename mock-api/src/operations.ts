...
const authRegister: OperationHandler = (context) => {
    ...
    context.store.state.users.push({
        ...
        enabled: !context.store.settings.requireEmailVerification,
        blocked: false
    });
    ...
    const user = context.store.state.users.find((candidate) => candidate.id === createdUserId)!;
    const verification = context.store.settings.requireEmailVerification ? issueVerification(context, user) : null;
    return { body: { deviceId: createdDeviceId, verification } };
};
...
function requireEnabledUser(user: UserRecord): void {
    if (user.blocked) throw new AppError(423, "Account is blocked");
...
function issueVerification(context: OperationContext, user: UserRecord) {
    const currentTime = Date.now();
    const previous = context.store.verificationDelivery.get(user.id);
    const sameWindow =
        previous !== undefined && currentTime < previous.windowStart + 3_600_000;
    let availableAt = previous ? previous.lastSent + 60_000 : currentTime;
    if (sameWindow && previous.sends >= 5)
        availableAt = Math.max(availableAt, previous.windowStart + 3_600_000);
    if (currentTime < availableAt) {
        verificationError(
            429,
            "Please wait before requesting another email",
            "RESEND_RATE_LIMITED",
            Math.ceil((availableAt - currentTime) / 1000)
        );
    }
    let verificationId: string;
    do {
        verificationId = randomUUID();
    } while (context.store.verifications.has(verificationId));
    const windowStart = sameWindow ? previous.windowStart : currentTime;
    const sends = sameWindow ? previous.sends + 1 : 1;
    const record = {
        verificationId,
        userId: user.id,
        code: "123456",
        token: randomBytes(32).toString("base64url"),
        expiresAt: new Date(currentTime + 900_000).toISOString(),
        resendAvailableAt: new Date(
            sends >= 5
                ? Math.max(currentTime + 60_000, windowStart + 3_600_000)
                : currentTime + 60_000
        ).toISOString(),
        retainedUntil: currentTime + 86_400_000,
        attempts: 0
    };
    context.store.verifications.set(verificationId, record);
    context.store.verificationDelivery.set(user.id, {
        id: verificationId,
...
function currentVerification(context: OperationContext, id: string) {
    const record = context.store.verifications.get(id);
    if (
        !record ||
        Date.now() >= record.retainedUntil ||
        context.store.verificationDelivery.get(record.userId)?.id !== id
    ) {
        verificationError(
            410,
    ...
    const user = context.store.state.users.find(
...
const authConfirmEmail: OperationHandler = (context) => {
    const payload = body(context) as {
        verificationId: string;
        code?: string;
        token?: string;
    };
    const { record, user } = currentVerification(context, payload.verificationId);
    ...
    const codeAttempt = payload.code !== undefined;
    const retryAfter = Math.ceil(
        (Date.parse(record.expiresAt) - Date.now()) / 1000
    );
    if (codeAttempt && user.enabled === false && record.attempts >= 5) {
        verificationError(
            429,
    ...
    if (
        ...
        if (codeAttempt && ++record.attempts >= 5) {
            verificationError(
                429,
                "Too many code attempts. Use the email link or request a new email",
                "TOO_MANY_ATTEMPTS",
    ...
    user.enabled = true;
    return { body: { message: "Email confirmed successfully" } };
};

const authResendVerification: OperationHandler = (context) => {
    const { user } = currentVerification(
        context,
        String(body(context).verificationId)
    );
    if (user.blocked) throw new AppError(423, "Account is blocked");
    return {
        body: {
            verification:
                user.enabled === false ? issueVerification(context, user) : null
        }
    };
};

const authRecoverVerification: OperationHandler = (context) => {
    const payload = body(context) as { identifier: string; password: string };
    const user = context.store.findUser(payload.identifier);
...
const authRefresh: OperationHandler = (context) => {
    ...
    const user = context.store.state.users.find((candidate) => candidate.id === refresh.userId);
...
export const operationHandlers: Record<string, OperationHandler> = {
    ...
    "auth.resendVerification": authResendVerification,
    "auth.recoverVerification": authRecoverVerification,
